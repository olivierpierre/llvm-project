...
#define PROCESSES  8 /* number of forked processes */
int pids[PROCESSES];
...
void handle_directory_request(int out_fd, int dir_fd, char *filename){
    ...
    if(!d) {
        printf("[%d] bad file descriptor %d\n", getpid(), dir_fd);
        __builtin_trap();
        return;
    }
    struct dirent *dp;
...
//    printf("[%d] got fd %d\n", getpid(), ffd);
    ...
    //    printf("[%d] closing %d\n", getpid(), ffd);
        ...
        /* handle_directory_request indirectly takes care of closing ffd */
        if(!dir) close(ffd);
    }
...
int main(int argc, char** argv){
    ...
    sleep(1);
    /* for the unit test we don't want this to run indefinitely ... */
    for(int i=0; i<(PROCESSES-1); i++)
        kill(pids[i], SIGTERM);
